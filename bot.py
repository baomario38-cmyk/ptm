...
@app.route('/')
def health_check():
    return "Bot Quốc Bảo AI Pattern Master v6.2 đang hoạt động 24/7!", 200
...
ADMIN_ID = 6363028212  # ID Admin
bot = telebot.TeleBot(TOKEN)

user_data = {}
all_user_ids = set()  # Tập hợp lưu danh sách toàn bộ ID người dùng
giftcodes = {}
admin_notice = "🔥 Hệ thống v6.2 đã cân bằng thuật toán MD5/SHA256! Chúc anh em rực rỡ!"

SUPPORTED_WEBS = ["HitClub", "B52", "Lucky88", "LC79"]

def init_user(uid):
    all_user_ids.add(uid)  # Tự động ghi nhớ ID vào hệ thống
...
def send_welcome(message):
    ...
    msg = (
        "👑 **TOOL QUỐC BẢO MD5 LUXURY** 👑\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        f"📢 **THÔNG BÁO ADMIN:**\n_{admin_notice}_\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        f"🆔 **ID:** `{uid}` | 🌐 **Cổng Game:** `{web}`\n"
        f"💰 **Số Xu hiện có:** **{xu} Xu** | 🔥 **Streak:** **{user_data[uid]['win_streak']} tay**\n"
        "🤖 **Thuật Toán:** `Exact Enumeration` + `Monte Carlo` + `DP`\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "💡 **HƯỚNG DẪN:** Dán thẳng mã **MD5 (32 ký tự)** hoặc **SHA256 (64 ký tự)** vào đây để soi!\n"
...
# --- 6. HỆ THỐNG QUẢN TRỊ ADMIN (GỬI THÔNG BÁO TỚI TẤT CẢ USER) ---
...
def broadcast_notice(message):
    ...
    try:
        # Lấy nội dung thông báo đằng sau lệnh
        notice_text = message.text.split(" ", 1)[1].strip()
    except IndexError:
        bot.reply_to(message, "❌ **Cú pháp sai!**\n👉 Dùng: `/thongbao <Nội_dung>`\n*Ví dụ:* `/thongbao Bảo trì hệ thống 10 phút, mọi người tạm dừng dùng tool!`, parse_mode="Markdown"")
        return

    # Cập nhật thông báo chung trên bảng tin /start
    global admin_notice
    admin_notice = notice_text
    
    # Bắt đầu gửi tin nhắn riêng cho tất cả người dùng
    success_count = 0
    fail_count = 0
...
def handle_all_messages(message):
    ...
    if len(text) in [32, 64]:
        ...
        res = (
            ...
            f"👑 **KẾT QUẢ SOI MÃ {code_type} (AI v6.2)** 👑\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 Dự đoán: **{res_label}**\n"
            f"📊 Phân tích: **Tài {percent_tai}% - Xỉu {percent_xiu}%**\n"
            f"⚡ Tỉ lệ chính xác AI: **{accuracy}%**\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
...
if __name__ == '__main__':
    ...
    print("Bot Master Ultimate v6.2 đang hoạt động...")
